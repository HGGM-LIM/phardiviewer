...
/*!
    ...
    \row
        ...
        \li While the left mouse button is pressed, mouse movement along x-axis pans the camera
        around the camera view center and movement along y-axis tilts it around the camera
        view center.
    \row
        \li Right mouse button
        \li While the right mouse button is pressed, mouse movement along x-axis moves the camera
...
void OrbitCameraControllerModified::init()
{
    //// Actions
    ...
    //Q_Q(OrbitCameraControllerModified);
    //// FrameAction
...
void OrbitCameraControllerModified::onTriggered(float dt)
{
    if (m_camera != nullptr) {
        // Mouse input
        ...
        if (m_altButtonAction->isActive()) {
            ...
            if ( zoomDistance(m_camera->position(), m_camera->viewCenter()) > m_zoomInLimit * m_zoomInLimit) {
                // Dolly up to limit
                m_camera->translate(QVector3D(0, 0, 1.5f*m_tzAxis->value()), m_camera->DontTranslateViewCenter);
            } else {
                m_camera->translate(QVector3D(0, 0, -0.5), m_camera->DontTranslateViewCenter);
            }
        }
...
OrbitCameraControllerModified::OrbitCameraControllerModified(Qt3DCore::QNode *parent)
...
{
    //Q_D(OrbitCameraControllerModified);
    init();
}

OrbitCameraControllerModified::~OrbitCameraControllerModified()
{
}

/*!
    \property OrbitCameraControllerModified::camera
    Holds the currently controlled camera.
*/
Qt3DRender::QCamera *OrbitCameraControllerModified::camera() const
{
    //Q_D(const OrbitCameraControllerModified);
    return m_camera;
}

/*!
    \property OrbitCameraControllerModified::linearSpeed
    Holds the current linear speed of the camera controller. Linear speed determines the
    movement speed of the camera.
*/
float OrbitCameraControllerModified::linearSpeed() const
{
    //Q_D(const OrbitCameraControllerModified);
...
/*!
    \property OrbitCameraControllerModified::zoomInLimit
    Holds the current zoom-in limit. The zoom-in limit determines how close to the view center
    the camera can be zoomed.
*/
float OrbitCameraControllerModified::zoomInLimit() const
{
    //Q_D(const OrbitCameraControllerModified);
    return m_zoomInLimit;
}

void OrbitCameraControllerModified::setCamera(Qt3DRender::QCamera *camera)
{
    //Q_D(OrbitCameraControllerModified);
    if (m_camera != camera) {

        if (m_camera)
            QObject::disconnect(m_camera);
...
void OrbitCameraControllerModified::setLinearSpeed(float linearSpeed)
{
    //Q_D(OrbitCameraControllerModified);
...
void OrbitCameraControllerModified::setLookSpeed(float lookSpeed)
{
    //Q_D(OrbitCameraControllerModified);
...
void OrbitCameraControllerModified::setZoomInLimit(float zoomInLimit)
{
    //Q_D(OrbitCameraControllerModified);
...
#include "moc_orbitcameracontrollermodified.cpp"
